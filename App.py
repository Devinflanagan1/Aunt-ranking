...
st.write(
    "Drag the ☰ handle to reorder. Move tiles close together to tie ranks!"
)

# Load global history for display
...
# Combined Drag-and-Drop + Irish Theme + Hidden bridge to Streamlit
...
<button class="btn" onclick="triggerSave()">📸 Save Global Snapshot</button>
...
<script>
    ...
    function triggerSave() {{
        ...
        // Send order back to Streamlit via URL parameters safely
        const encodedOrder = encodeURIComponent(currentOrder.join(','));
        window.top.location.href = window.location.origin + window.location.pathname + "?save_order=" + encodedOrder;
    }}
</script>
</body>
</html>
...
# Check if save was triggered from the UI component
if "save_order" in st.query_params:
  order_str = st.query_params.get("save_order", "")
  if order_str:
...
# Render the styled drag-and-drop interface
